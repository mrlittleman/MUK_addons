...
class RESTController(http.Controller):
    ...
    @http.route('/api/<path:path>', auth="none", type='http', cors="*")
    def catch(self, **kw):    
        return Response(json.dumps(NOT_FOUND,
                        sort_keys=True, indent=4, cls=ObjectEncoder),
                        content_type='application/json;charset=utf-8', status=404) 
    
    @http.route('/api', auth="none", type='http', cors="*")
    def version(self, **kw):    
       return Response(json.dumps(REST_VERSION,
                        sort_keys=True, indent=4, cls=ObjectEncoder),
                        content_type='application/json;charset=utf-8', status=200) 
    
    @http.route('/api/authenticate', auth="none", type='http', methods=['POST', 'OPTIONS'], csrf=False, cors="*")
